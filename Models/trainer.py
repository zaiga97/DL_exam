...
def train(g: Generator, d: Discriminator, dataloader_gen: DataloaderGenerator, iterations_schedule: List[int],
    ...
    for level in range(0, max_level):
        ...
        for i in pbar:
            ...
            # Fake data
            z = torch.randn(batch_size, g.input_size, 1, 1).to(device)
            ...
            d_loss = fake_predict - real_predict
            d_loss.backward()
            d_opt.step()
            ...
            if log_dir is not None:
                if (i + 1) % 1000 == 0:
                    ...
                    with torch.no_grad():
                        images = g(torch.randn(5 * 10, g.input_size, 1, 1).to(device), level=level, alpha=alpha).data.cpu()
                        save_image(
                            images,
                ...
                if (i + 1) % 500 == 0:
                    ...
                    f = open(log_file, 'a+')
                    f.write(f"{cum_d_losses/500},{cum_g_losses/500}\n")
                    f.close()
                    cum_g_losses = 0
                    cum_d_losses = 0
