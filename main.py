...
if __name__ == '__main__':
    ...
    log_dir = f'./ProGAN_run_{date_time.date()}_{date_time.hour}:{date_time.minute}'
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
        os.makedirs(log_dir + '/images')
        os.makedirs(log_dir + '/model')

    g = Generator(input_size=opts.latent_dim, hidden_channels=opts.hidden_dim)
    d = Discriminator(hidden_channels=opts.hidden_dim)
